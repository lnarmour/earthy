...
mysql -h 127.0.0.1 -u $USERNAME -p$PASSWORD -e "create database $DATABASE_NAME" &> /dev/null
check_return $? "Something went wrong while creating database." "Database successfully created."

mysql -h 127.0.0.1 -u $USERNAME -p$PASSWORD $DATABASE_NAME < res/schema &> /dev/null
